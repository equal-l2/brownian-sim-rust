...
impl ProgressBar {
    ...
    pub fn adv_by(&mut self, step: u64) {
        self.count += step;
        if self.count >= self.next_show_count {
            self.show();
        }
    }
    ...
    fn show(&mut self) {
        ...
        let next_ratio_count = if self.precision > 0 {
            self.count + (max / 10_f64.powi(2 + self.precision)) as u64
        } else {
            self.count + self.max / 100
        };
        let pcent_width = if self.precision > 0 {
            4 + self.precision
        } else {
            3
        } as usize;
        ...
        self.next_show_count = *[next_tic_count, next_ratio_count, self.max - 1]
            .iter()
            .min()
            .unwrap();
